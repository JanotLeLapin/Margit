...
public final class CraftServer implements Server {
    ...
    private final Yaml yaml = new Yaml(new SafeConstructor(new LoaderOptions()), new Representer(new DumperOptions()), new DumperOptions(), loaderOptions); // Margit
    // KigPaper end
    ...
    */ // KigPaper - no longer needed
    ...
    public LineReader getReader() { // Margit
    ...
    public void configureDbConfig(ServerConfig config) {
        ...
        // Margit start
        TxIsolation isolation = switch (configuration.getString("database.isolation")) {
            case "SERIALIZABLE" -> TxIsolation.SERIALIZABLE;
            case "DEFAULT" -> TxIsolation.DEFAULT;
            case "READ_COMMITED" -> TxIsolation.READ_COMMITED;
            case "READ_UNCOMMITED" -> TxIsolation.READ_COMMITED;
            case "REPEATABLE_READ" -> TxIsolation.REPEATABLE_READ;
            default -> TxIsolation.NONE;
        };
        ds.setIsolationLevel(isolation.getLevel());
        // Margit end

        if (ds.getDriver().contains("sqlite")) {
            config.setDatabasePlatform(new SQLitePlatform());
            /* Margit, don't know how to refactor this
    ...
    public OfflinePlayer getOfflinePlayer(String name) {
        Validate.notNull(name, "Name cannot be null");
        com.google.common.base.Preconditions.checkArgument( !org.apache.commons.lang3.StringUtils.isBlank( name ), "Name cannot be blank" ); // Spigot // Margit
